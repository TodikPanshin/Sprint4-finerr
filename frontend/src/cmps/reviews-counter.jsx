    ...
    <ul className="star-progress-container clean-list">
      {reviewsCount.map((ratingBar) => (
        <li className="star-progress" key={ratingBar.rating}>
          ...
          <span className="star-progress-progress-bar"> <ProgressBar completed={checkCount(ratingBar.count,reviewsNum)} />
          </span>
          <span className="star-progress-count">{`(${ratingBar.count})`}
          </span>
        </li>))}
    </ul>
  )
